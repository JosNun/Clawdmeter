...
// ---- Mini creatures: small animated creatures embedded in other screens
//      (idle indicator, pairing hint, ...). Each is self-contained — its own
//      canvas and buffer, independent of the full-screen splash above. A small
...
void splash_mini_tick(void) {
    for (int i = 0; i < mini_count; i++) {
        splash_mini_t *m = &minis[i];
        if (!m->buf || !m->anim || m->anim->frame_count == 0) continue;
        // Skip hidden creatures (off-screen state) so they don't burn frames;
        // they resume from frame 0 next time their screen is shown.
        if (m->canvas && !lv_obj_is_visible(m->canvas)) { m->frame = 0; m->started = millis(); continue; }
